...
const Navbar = ({ setActive, isActive }) => {
    ...
    useEffect(() => {
        ...
        if (isInfoPage) {
            ...
            else if (infoSegment?.startsWith("add-")) {
                // find child menu item matching this path
                const childItem = parentMenu?.children?.find(c =>
                    c.path.split("/").pop() === infoSegment
                );

                // prefer child's "add" title, fallback to parent, then fallback to default
                subTitle = childItem?.add || parentMenu?.add || "Add";
            }
